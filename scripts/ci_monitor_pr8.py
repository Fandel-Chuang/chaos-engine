...
for attempt in range(40):
    ...
    for r in data.get('workflow_runs', []):
        if r.get('head_branch') == 'perf/gateway-10k-qps-v2':
            found = True
            run_id = r['id']
            ...
            if status == 'completed':
                ...
                if all_ok:
                    print('=== CI ALL PASSED ===')
                    merge_url = f'https://api.github.com/repos/{repo}/pulls/8/merge'
                    merge_data = json.dumps({
                        'commit_title': 'perf(gateway): 万并发 QPS 14.8K -> 97.5K (6.6x) (#8)',
                        'merge_method': 'squash'
                    }).encode()
