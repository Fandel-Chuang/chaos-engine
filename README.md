...
<div style="font-size:11px;color:#b45309;">TCP / KCP / WebSocket 共用端口 9000</div>
</div>
</div>
</div>
...
| 网关 | `chaos_gateway` | TCP / KCP / WebSocket | 9000 | 协议路由 + 连接管理（三协议共用端口） |
...
`chaos_gateway` 是集群的统一入口，**纯 C 实现**（`src_c/gateway/`），负责协议路由、连接管理和心跳维护。Gateway 在同一个端口（9000）上同时绑定 TCP / KCP / WebSocket 三种协议，利用内核 TCP/UDP 端口空间隔离特性，自动识别连接类型并路由到对应处理逻辑。

**Gateway C 内核模块：**

| 模块 | 源文件 | 职责 |
|------|--------|------|
| 核心入口 | `ce_gateway.c` | 事件循环 + 三协议监听（TCP+KCP 共用 9000 端口） |
| 连接管理 | `ce_gateway_conn.c` | 连接生命周期 + 会话状态机 |
| 协议处理 | `ce_gateway_protocol.c` | 消息解析 + 协议适配 |
| 后端路由 | `ce_gateway_backend.c` | 到 Game Server(7777) 的 TCP 长连接 |
| 内部路由 | `ce_gateway_router.c` | 消息分发 + 负载均衡 |

**端口复用机制：** TCP 和 KCP(UDP) 共用端口 9000，因为 Linux 内核按协议类型隔离端口空间，互不冲突。WebSocket 在同端口上通过 HTTP Upgrade 握手升级。
...
> **注意：** `chaos_gateway` 是纯 C 实现（`src_c/gateway/`），不走 Lua 服务化。Router / DBProxy / Admin / Services 使用 Lua 脚本驱动。
...
# 启动: game(7777) + gateway(9000) + dbproxy(9003) + router(9100) + admin(9090)
