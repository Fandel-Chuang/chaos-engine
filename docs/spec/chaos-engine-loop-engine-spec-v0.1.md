...
执行流程:
  ...
  8. ProcMgr: ./scripts/stop_cluster_server.sh (清理)
...
| VerifyLoop | `stop_cluster_server.sh` | 停止集群 | 同步等待 | 全部停止 |
...
| ClientUILoop | `stop_cluster_server.sh` | 停止客户端 | 清理 | 进程退出 |
...
| **集群进程残留导致端口冲突** | 中 | 低 | stop_cluster_server.sh 强制清理；启动前检查端口占用 |
