...
2. **Router 服务注册与发现**：微服务进程启动时向 Router 注册，Gateway 从 Router 动态查询路由表。
3. **一致性哈希消息路由**：按 player_id 一致性哈希将请求路由到目标微服务进程，支持进程扩缩容时的自动再均衡。
4. **Router 集群**：区域内多 Router 实例组成集群，广播同步路由表，避免单点故障。
5. **跨区域转发**：不同物理区域的 Router 通过 TCP 互联，支持全球同服的跨区域消息路由。
...
│  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐                        │    │
│  │  │ 连接管理       │  │ 协议适配      │  │ 消息路由      │                        │    │
│  │  │ (ce_net_base) │  │ TCP/KCP/WS   │  │ (查Router表)  │                        │    │
│  │  └──────────────┘  └──────────────┘  └──────┬───────┘                        │    │
│  └─────────────────────────────────────────────┼─────────────────────────────────┘    │
│                                                │                                      │
│                         路由表查询 + 消息转发    │                                      │
...
│  │                                                                              │    │
│  │  ┌──────────────────┐   ┌──────────────────┐   ┌──────────────────┐          │    │
│  │  │  Router R1 (Lua) │◄──►  Router R2 (Lua) │◄──►  Router R3 (Lua) │          │    │
...
│                                   │                                                  │
│  ┌────────────────────────────────┴─────────────────────────────────────────────┐    │
│  │                     Router 集群 (同 us-west 结构)                              │    │
│  └────────────────────────────────┬─────────────────────────────────────────────┘    │
│                                   │                                                  │
│  ┌────────────────────────────────┼─────────────────────────────────────────────┐    │
│  │  Gateway ← → 微服务层 (Game/FRIEND/PVP/PVE/TRADE/GUILD/CHAT/MAIL)             │    │
│  │                              DBProxy ← → MongoDB                               │    │
│  └──────────────────────────────────────────────────────────────────────────────┘    │
└──────────────────────────────────────────────────────────────────────────────────────┘
```
...
├──────────────────────────────────────────────────────────────────┤
│                    Lua 业务层                                      │
│  src_lua/gateway/   (连接管理、协议适配、路由查询)                   │
...
- **统一协议**：Gateway 和 Router 使用相同的二进制协议格式，客户端 → Gateway → Router → 微服务的消息无需协议转换。
- **统一维护**：网络层的 bug 修复和性能优化只需在一处进行，Gateway 和 Router 同时受益。
- **Gateway 重构友好**：Gateway 原本内联的网络逻辑替换为 ce_net_base 调用，Gateway 代码量减少 ~40%，聚焦于协议适配和路由查询。
...
### Decision 4: 跨区域 Router 互联 vs 跨区域消息队列

**选择**：Router 间直接 TCP 长连接，全互联拓扑。

**理由**：
- **低延迟**：直连 TCP 比经过消息队列（Kafka/RabbitMQ）少一跳，延迟更低。
- **部署简单**：无需额外的消息队列集群，减少运维负担。
- **全互联可控**：区域数量通常较少（3-5 个），全互联连接数可控（N*(N-1)/2 条连接）。
- **故障隔离**：单个跨区域连接断开不影响其他区域通信。
...
**选择**：所有微服务间消息通过 Router 中转，微服务之间不建立直接连接。

**理由**：
- **集中路由管控**：Router 统一管理消息路由，便于监控、限流、审计。
...
客户端          Gateway           Router            微服务(FRIEND)      DBProxy(FRIEND)
──────          ───────           ──────            ──────────────      ───────────────
   │               │                 │                    │                   │
   │ ──好友请求──→  │                 │                    │                   │
   │ {msg_type:    │                 │                    │                   │
...
Router R1 (us-west)                      │
   ...
   │ ──跨区域TCP转发──────────────────→  │
   │ {src_region: us-west,               │
   │  dst_region: ap-southeast,          │
   │  player_id: 99,                     │
   │  body: {action: "pvp_challenge"}}   │
   │                                     ▼
   │                              Router R4 (ap-southeast)
   │                                     │
...
- **缓解**：
  - Router 集群多实例部署，Gateway 和微服务可连接不同 Router 实例分散负载。
...
- **选择**：通过 Router 集中路由。
- **代价**：增加一跳网络延迟（~0.1-1ms 内网），Router 成为关键路径。
- **收益**：统一服务发现、集中管控、简化微服务、连接数从 O(N²) 降为 O(N)。
