...
新增 Router 进程，作为服务注册中心和 **Game↔Game 消息路由层**。Router 的职责范围仅限于 Game 进程之间的通信路由，不参与客户端消息路径（客户端 → Gateway → Game 是直连的，不经过 Router）。Router 负责服务注册与发现（Game 进程启动时注册，其他 Game 进程查询）、Game↔Game 消息路由（按 player_id 一致性哈希路由到目标 Game 进程）、**跨大区路由（核心能力，Router 间 TCP 长连接组成全球 Router 网格）**、健康检查与故障剔除、路由表同步（Router 集群内广播）。Router 以独立的 Lua 进程运行，网络层使用共享 C 库 `ce_net_base`。
...
Router SHALL 作为服务注册中心，管理所有后端 Game 进程的注册信息，并提供查询接口。

服务注册 MUST 满足以下要求：
- Game 进程启动时 MUST 向 Router 发送注册请求，包含服务类型、进程 ID、监听地址和端口
...
- Game 进程 MUST 能通过 Router 查询服务注册表，获取目标服务的地址列表（用于 Game↔Game 通信）
- Game 进程正常关闭时 MUST 向 Router 发送注销请求
- Router MUST 支持服务注册表的全量和增量查询

#### Scenario: Game 进程注册

- **WHEN** 好友 Game 进程（service_type=FRIEND, process_id=2001）启动并连接到 Router
...
#### Scenario: Game 进程查询路由表

- **WHEN** 好友 Game 进程（FRIEND/2001）需要向 GAME 进程发送消息，查询 GAME 进程的可用实例
- **THEN** 好友 Game 进程向 Router 发送 QUERY 请求：`{service_type: "GAME"}`
- **AND** Router 返回所有 ONLINE 状态的 GAME 进程列表：`[{process_id: 1001, address: "192.168.1.10:7777"}, {process_id: 1002, address: "192.168.1.11:7777"}]`
- **AND** 好友 Game 进程缓存查询结果，TTL 为 30 秒

#### Scenario: Game 进程注销

- **WHEN** 好友 Game 进程正常关闭（收到 SIGTERM）
...
- **AND** 已缓存该进程信息的 Game 进程在 TTL 过期后获取更新列表

---

### Requirement: 消息路由（一致性哈希）

Router SHALL 使用一致性哈希算法将 **Game↔Game 消息**路由到目标 Game 进程，确保同一玩家的请求始终到达同一进程。**Router 不处理客户端消息路由（客户端 → Gateway → Game 是直连的）。**

消息路由 MUST 满足以下要求：
- 路由键为 `player_id`，使用一致性哈希（虚拟节点数 150）映射到目标 Game 进程
...
#### Scenario: 一致性哈希路由（Game↔Game）

- **WHEN** 好友服务（FRIEND/2001）需要查询 player_id=42 的在线状态，向 Router 发送路由请求
- **THEN** Router 对 player_id=42 计算哈希值，在一致性哈希环上查找最近的虚拟节点
- **AND** 虚拟节点映射到 GAME 服务进程 process_id=1001
- **AND** Router 将消息转发到 192.168.1.10:7777
- **AND** 后续 player_id=42 的所有 Game↔Game 请求均路由到 process_id=1001
...
- **AND** 数据迁移由上层 Game 进程处理（Router 不负责数据迁移）

#### Scenario: 直接路由模式

- **WHEN** Game 进程 A 需要向特定 Game 进程 B（process_id=3001）发送消息
- **THEN** Game 进程 A 在消息中指定目标 process_id=3001
...
- **THEN** 每个大区独立部署完整集群（Gateway + Router 集群 + Game 进程 + DBProxy + MongoDB）
...
- **AND** 响应沿原路径返回：PVP 进程 → ap-southeast Router → us-west Router → Game 进程 → Gateway → 玩家 A
...
Router SHALL 对所有注册的 Game 进程执行健康检查，自动剔除故障进程。

健康检查 MUST 满足以下要求：
- Router 每 1 秒向所有注册的 Game 进程发送 PING 心跳
- Game 进程 MUST 在收到 PING 后立即回复 PONG
...
#### Scenario: Game 进程故障检测

- **WHEN** 好友 Game 进程 process_id=2001 崩溃
...
- **WHEN** 好友 Game 进程 process_id=2001 恢复运行并重新向 Router 发送 REGISTER
