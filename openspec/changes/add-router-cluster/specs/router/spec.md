...
新增 Router 进程，作为服务注册中心和 **Game↔Game 消息路由层**。Router 的职责范围仅限于微服务之间的通信路由，不参与客户端消息路径（客户端 → Gateway → Game 是直连的，不经过 Router）。Router 负责服务注册与发现（Game 进程启动时注册，其他 Game 进程查询）、Game↔Game 消息路由（按 player_id 一致性哈希路由到目标微服务进程）、**跨大区路由（核心能力，Router 间 TCP 长连接组成全球 Router 网格）**、健康检查与故障剔除、路由表同步（Router 集群内广播）。Router 以独立的 Lua 进程运行，网络层使用共享 C 库 `ce_net_base`。
...
- 微服务 MUST 能通过 Router 查询服务注册表，获取目标服务的地址列表（用于 Game↔Game 通信）
...
- **WHEN** 好友服务（FRIEND/2001）需要向 GAME 服务发送消息，查询 GAME 服务的可用进程
- **THEN** 好友服务向 Router 发送 QUERY 请求：`{service_type: "GAME"}`
- **AND** Router 返回所有 ONLINE 状态的 GAME 服务进程列表：`[{process_id: 1001, address: "192.168.1.10:7777"}, {process_id: 1002, address: "192.168.1.11:7777"}]`
- **AND** 好友服务缓存查询结果，TTL 为 30 秒
...
- **AND** 已缓存该进程信息的微服务在 TTL 过期后获取更新列表

---

### Requirement: 消息路由（一致性哈希）

Router SHALL 使用一致性哈希算法将 **Game↔Game 消息**路由到目标微服务进程，确保同一玩家的请求始终到达同一进程。**Router 不处理客户端消息路由（客户端 → Gateway → Game 是直连的）。**
